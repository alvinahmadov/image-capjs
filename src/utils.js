...
function printData(data, lineLen = 10, name = null) {
	let out
...
function readParams(file_path, defaultKey = 'models') {
...
async function readWeightMaps(manifest, prefix = '') {
...
function rowMax(tensor, rowIndex) {
 ...
 * @param {string} dtype
 * */
function mulScalar(tensor, scalar, dtype = "int32") {
...
function saveModelAsJSON(path, model) {
	jsonfile.writeFile(
		path, model.toJSON(), () => console.log("Model save finished.")
	);
}

function loadModelFromJSON(path) {
	let object = null;
	try {
